...
const swRes=await fetch(`${BASE}admin/sw.js`);ok(swRes.ok,`service worker missing: ${swRes.status}`);const sw=await swRes.text();ok(sw.includes("url.origin!==self.location.origin"),'service worker origin guard missing');ok(sw.includes("/functions/")&&sw.includes("/rest/")&&sw.includes("/auth/"),'service worker API/auth cache exclusions missing');ok(sw.includes("dc-admin-shell-v4"),'service worker cache version not updated');ok(sw.includes('FRESH_ASSETS'),'fresh asset policy missing');ok(!sw.includes("'./view-router.js'"),'obsolete view router should not be pre-cached');
