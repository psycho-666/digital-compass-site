...
self.addEventListener('fetch',event=>{
  ...
  if(req.mode==='navigate'){
    event.respondWith(fetch(req).then(res=>{if(res.ok){const copy=res.clone();caches.open(CACHE).then(c=>c.put('./',copy))}return res}).catch(()=>caches.match('./')));return;
  }
  const name=url.pathname.split('/').pop()||'';
  if(FRESH_ASSETS.has(name)){
...
self.addEventListener('push',event=>{
  let data={};try{data=event.data?.json()||{}}catch{data={body:event.data?.text()||'مطلوب إجراء منك'}}
...
self.addEventListener('notificationclick',event=>{
  event.notification.close();const raw=event.notification.data?.action_path||'./';const target=new URL(raw,self.registration.scope).href;
  event.waitUntil(self.clients.matchAll({type:'window',includeUncontrolled:true}).then(async clients=>{for(const client of clients){if(client.url.startsWith(self.registration.scope)){try{await client.navigate(target)}catch{}return client.focus()}}return self.clients.openWindow(target)}));
});