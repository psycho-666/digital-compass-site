...
const swRes=await fetch(`${BASE}admin/sw.js`);ok(swRes.ok,`service worker missing: ${swRes.status}`);const sw=await swRes.text();ok(sw.includes("url.origin!==self.location.origin"),'service worker origin guard missing');ok(sw.includes("/functions/")&&sw.includes("/rest/")&&sw.includes("/auth/"),'service worker API/auth cache exclusions missing');ok(sw.includes("dc-admin-shell-v3"),'service worker cache version not updated');ok(sw.includes('FRESH_ASSETS'),'fresh asset policy missing');ok(!sw.includes("'./view-router.js'"),'obsolete view router should not be pre-cached');
...
try{
 for(const [label,viewport] of [['desktop',{width:1440,height:1000}],['mobile',{width:390,height:844}]]){
   ...
   await page.goto(`${BASE}admin/`,{waitUntil:'networkidle',timeout:30000});await page.waitForSelector('.secureAccessButton',{state:'visible',timeout:10000});
   ok(await page.locator('#loginForm').count()===0,`${label}: legacy password login form should be removed`);ok(await page.locator('#setupForm').count()===0,`${label}: legacy setup form should be removed`);
...
console.log('PASS manifest, passwordless access, installability and service-worker safety checks');