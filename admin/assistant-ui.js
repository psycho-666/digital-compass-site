...
<section id="dcAssistantPanel" class="dcPanel dcAssistantPanel hidden" aria-label="Digital Compass Assistant">
 ...
 <div class="dcAssistantIntro"><strong>اسأل عن النظام أو اطلب إجراءً واضحاً.</strong><p>المساعد الحالي متخصص بتشغيل البوصلة وقراءة حالتها، وليس بديلاً عن ChatGPT العام. الإجراءات الحساسة تبقى خاضعة للموافقة.</p></div>
...
<section id="dcNotifyPanel" class="dcPanel hidden" aria-label="Notifications">
 ...
 <div class="dcPanelTabs"><button data-notify-tab="notifications">التنبيهات</button><button data-notify-tab="approvals">الموافقات</button></div>
...
function bind(){
 ...
 $('#dcNotifyBtn')?.addEventListener('click',async()=>{toggle('#dcNotifyPanel');if(!$('#dcNotifyPanel').classList.contains('hidden'))await loadState(true,true)});
 $('[data-close-assistant]').onclick=()=>$('#dcAssistantPanel').classList.add('hidden');$('[data-close-notify]').onclick=()=>$('#dcNotifyPanel').classList.add('hidden');
 ...
 document.querySelectorAll('[data-notify-tab]').forEach(b=>b.onclick=()=>{selectTab(b.dataset.notifyTab);loadState(true,false)});
...
async function send(text){addMsg('user',text);$('#dcChatForm button').disabled=true;try{const d=await api('message',{method:'POST',body:{conversation_id:conversationId||null,content:text}});conversationId=d.conversation_id;localStorage.setItem('dc_assistant_conversation',conversationId);addMsg('assistant',d.message?.content||d.result?.text||'تم.');await loadState(false,false)}catch{addMsg('assistant','تعذر تنفيذ الطلب الآن. إذا كان متعلقاً ببيانات أو تشغيل افتح القسم المعني وتأكد من الحالة.')}finally{$('#dcChatForm button').disabled=false}}
...
function approvalsHtml(items){if(!items?.length)return '<div class="dcEmpty">ما عندك أي موافقة معلّقة حالياً.</div>';return items.map(x=>`<article class="dcApproval ${x.risk_level==='CRITICAL'?'critical':''}"><div class="dcApprovalTop"><div><h4>${esc(x.title)}</h4><p>${esc(x.description||'')}</p></div><span class="dcRisk">${esc(x.risk_level)}</span></div><div class="dcApprovalActions"><button class="dcApprove" data-approve="${x.id}">موافق</button><button class="dcReject" data-reject="${x.id}">رفض</button></div></article>`).join('')}
function notificationsHtml(items){if(!items?.length)return '<div class="dcEmpty">ما في تنبيهات بعد.</div>';return items.map(x=>`<article class="dcNotification ${x.is_read?'':'unread'}" data-notification="${x.id}"><h4>${esc(x.title)}</h4><p>${esc(x.body||'')}</p><time>${formatTime(x.created_at)}</time>${x.is_read?'':'<small>اضغط لاعتباره مقروءاً</small>'}</article>`).join('')}
async function loadState(render=false,autoTab=false){try{const d=await api('state',{params:conversationId?{conversation_id:conversationId}:{}}),badge=$('#dcNotifyBadge');badge.textContent=d.unread_count||0;badge.classList.toggle('hidden',!(d.unread_count>0));if(d.unread_count>knownUnread&&'Notification'in window&&Notification.permission==='granted'){const newest=(d.notifications||[]).find(x=>!x.is_read);if(newest)new Notification(newest.title,{body:newest.body||'مطلوب إجراء منك'})}knownUnread=d.unread_count||0;localStorage.setItem('dc_known_unread',knownUnread);
 if(render&&!$('#dcNotifyPanel').classList.contains('hidden')){if(autoTab){if(d.unread_count>0)selectTab('notifications');else if((d.approvals||[]).length)selectTab('approvals');else selectTab('notifications')}let tab=document.querySelector('[data-notify-tab].active')?.dataset.notifyTab;if(!tab){tab=d.unread_count>0?'notifications':'approvals';selectTab(tab)}$('#dcNotifyBody').innerHTML=tab==='approvals'?approvalsHtml(d.approvals):notificationsHtml(d.notifications);bindNotificationRows()}
 if(d.messages?.length&&$('#dcChatMessages').children.length<=1){$('#dcChatMessages').innerHTML='';d.messages.forEach(m=>addMsg(m.role==='USER'?'user':'assistant',m.content))}return d}catch{return null}}
...
async function resolve(id,decision){try{await api('resolve_approval',{method:'POST',body:{id,decision}});await loadState(true,false)}catch{alert('تعذر تحديث الموافقة الآن.')}}
