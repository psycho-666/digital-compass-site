...
function updateCounters(d){const unread=Number(d?.unread_count||0);const pending=Number(d?.pending_approvals??(d?.approvals?.length||0));const attention=Number(d?.attention_count??(unread+pending));const badge=$('#dcNotifyBadge');badge.textContent=attention>99?'99+':attention;badge.classList.toggle('hidden',attention<=0);const n=$('[data-notify-count]'),a=$('[data-approval-count]');if(n)n.textContent=unread;if(a)a.textContent=pending;const summary=$('#dcActionSummary');if(summary)summary.textContent=attention?`${pending} موافقة معلّقة • ${unread} تنبيه غير مقروء`:'لا يوجد شيء يحتاج انتباهك الآن';const mark=$('#dcMarkAllRead');if(mark)mark.disabled=unread===0}
